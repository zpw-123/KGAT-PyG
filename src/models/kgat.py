...
class KGAT(torch.nn.Module):
    ...
    def __init__(self, data, args):
        ...
        for layer in range(len(self.conv_layer_dim_list)-1):
            self.convs.append(
                KGATConv(self.conv_layer_dim_list[layer], self.conv_layer_dim_list[layer+1], aggr_type=self.aggr_type, node_dropout=self.node_dropout)
                )
